...
const SubCategoriesRequest = () => {
  ...
  medicine_subcat_request_delete: true,
  accessAll: true,
};
  ...
  renderCell: (params) => (
    <Box display="flex" gap={1}>
      {(rolesAndPermission.medicine_subcat_request_edit === true ||
        rolesAndPermission.accessAll === true) &&
        params.row.isRequested !== true && (
          <>
            {/* âœ… Approve Button */}
            <Button
              size="small"
      ...
      {(rolesAndPermission.medicine_subcat_request_delete === true ||
        rolesAndPermission.accessAll === true) && (
        <Button
          size="small"
          color="error"
