...
const changeHandler = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append('flagType', 'medicine');
...
const EditCategoryForm = ({ onClose, onSubmit, initialData, loading }) => {
  ...
  const [iconFile, setIconFile] = useState(null);
const [iconPreview, setIconPreview] = useState(initialData?.categoryIcon ? URLS.FileBase + initialData.categoryIcon : null);
...
const changeHandler = (e) => {
  ...
  if (selectedFile) {
    ...
    if (ext === "zip") {
      // ZIP is allowed but no preview
      setFile(selectedFile);
      setPreview(null);
      return;
    }

    // If file is an IMAGE (browser-detectable)
    if (selectedFile.type.startsWith("image/")) {
      setFile(selectedFile);
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.medicine_categories_edit === true ||
            ...
            {rolesAndPermission.medicine_categories_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            />
            {rolesAndPermission.medicine_categories_add === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  variant="contained"
