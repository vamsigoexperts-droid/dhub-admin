...
function Brands() {
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.AddBrand, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    axios
      .put(`${URLS.EditBrand}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      setLoading(true);
      axios
        .delete(`${URLS.DeleteBrand}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast.success(res.data.message);
  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => setData(res.data.brands || []))
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" gap={1}>
          {rolesAndPermission.medicine_brands_edit === true ||
          ...
          {rolesAndPermission.medicine_brands_delete === true ||
          rolesAndPermission.accessAll === true ? (
            <>
              <Button
                size="small"
    ...
    <PageContainer title="Brands" description="this is Brands page">
      ...
      {showEdit && (
        <form onSubmit={handleEditSubmit}>
          <ParentCard title="Edit Brand" sx={{ overflowX: 'hidden' }}>
            <Grid container rowSpacing={3}>
              <Grid item xs={6} sx={{ padding: 1 }}>
                <CustomFormLabel htmlFor="name">
                  Brand Name <span style={{ color: 'red' }}>*</span>;
        ...
        <CardHeader
          ...
          action={
            <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
              ...
              />{' '}
              {rolesAndPermission.medicine_brands_add === true ||
              rolesAndPermission.accessAll === true ? (
                <>
                  <Button
                    variant="contained"
