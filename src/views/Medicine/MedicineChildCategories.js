    ...
    const fetchSubcategories = async () => {
      ...
      try {
        const res = await axios.post(
          ...
          { categoryId: form.categoryId, flagType: 'medicine' },
...
const EditCategoryForm = ({ onClose, onSubmit, initialData, categories, loading }) => {
  ...
  const [form, setForm] = useState({
    ...
    flagType: initialData?.flagType || 'medicine',
  });
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      ...
      try {
        const res = await axios.post(
          URLS.GetCategorieIdbySubCategory,
          { categoryId: categoryId, flagType: 'medicine' },
          { headers: { Authorization: `Bearer ${token}` } },
        );
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.medicine_child_categories_edit === true ||
            ...
            {rolesAndPermission.medicine_child_categories_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            />{' '}
            {rolesAndPermission.medicine_child_categories_add === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  variant="contained"
