...
const AddMedicineProduct = () => {
  ...
  const fetchScheduleDrugCategories = useCallback(() => {
    fetchData(URLS.GetDrugCategories, 'drugcategory', setScheduleDrugCategories, 'drugcategory', {
      flagType: 'medicine',
    });
  }, [fetchData]);

  const fetchPackagingTypes = useCallback(() => {
    fetchData(URLS.GetPackingTypes, 'packingtype', setPackagingTypes, 'packingtype', {
      flagType: 'medicine',
    });
  }, [fetchData]);

  const fetchForms = useCallback(() => {
    fetchData(URLS.GetMedicineForms, 'medicineform', setForms, 'medicineform', {
      flagType: 'medicine',
    });
  }, [fetchData]);
...
const handleKeywordKeyDown = useCallback((e, field) => {
  ...
  const removeKeyword = useCallback((index, field) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleFileUpload = useCallback(
    (e, type) => {
      ...
      if (type === 'productImage' || type === 'additionalImages') {
        if (!file.type.startsWith('image/')) {
          toast.error('Only image files are allowed');
          return;
        }
      } else if (type === 'instructionLeaflet' && file.type !== 'application/pdf') {
        toast.error('Only PDF files are allowed for instruction leaflet');
        return;
      } else if (type === 'prescriptionSample' && !file.type.startsWith('image/')) {
        toast.error('Only image files are allowed for prescription sample');
        return;
      }
  ...
  const validate = useCallback(() => {
    ...
    if (!formData.manufacturerBrandId)
      newErrors.manufacturerBrandId = 'Manufacturer/Brand selection is required';
    ...
    if (formData.tagsKeywords.length === 0)
      newErrors.tagsKeywords = 'At least one tag/keyword is required';

    // Optional Fields (No validation needed unless specified)
    // prescriptionRequired, freeShippingEligible, codAvailable are boolean switches, no validation needed
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const response = await axios.post(URLS.AddMedicalItem, fd, {
        ...
        if (response.data?.success) {
          ...
          throw new Error(response.data?.message || 'Failed to add medicine product');
        }
      } catch (error) {
        console.error('Error submitting form:', error);
        let errorMessage = 'Failed to add medicine product. Please try again.';
  ...
  const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Add Medicine Product' }];
  ...
  return (
    <PageContainer title="Add Medicine Product">
      <Breadcrumb title="Add Medicine Product" items={BCrumb} />
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ParentCard title="Basic Medicine Information" sx={{ mt: 3 }}>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <CustomFormLabel htmlFor="medicineName">
            ...
            <Grid item xs={12} md={6}>
              ...
              <CustomSelect
                ...
                error={!!errors.manufacturerBrandId}
                aria-describedby={
                  errors.manufacturerBrandId ? 'manufacturerBrandId-error' : undefined
                }
              >
              ...
              {errors.manufacturerBrandId && (
                <FormHelperText error id="manufacturerBrandId-error">
                  {errors.manufacturerBrandId}
                </FormHelperText>
              )}
            </Grid>
            ...
            <Grid item xs={12}>
              <CustomFormLabel htmlFor="tagsKeywords" required>
              ...
              >
                {formData.tagsKeywords.map((keyword, index) => (
                  <Chip
                    key={index}
                    label={keyword}
                    onDelete={() => removeKeyword(index, 'tagsKeywords')}
                    sx={{ mr: 0.5 }}
                ...
                <CustomTextField
                  variant="standard"
                  onChange={(e) => handleKeywordChange(e, 'tagsKeywords')}
                  onKeyDown={(e) => handleKeywordKeyDown(e, 'tagsKeywords')}
              ...
              {errors.tagsKeywords && <FormHelperText error>{errors.tagsKeywords}</FormHelperText>}
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <Card sx={{ mb: 3 }}>
              <CardHeader title={`Additional Images (${additionalImages.length}/5) *`} />
              <Divider />
              <CardContent sx={{ p: 3 }}>
                <Stack spacing={2}>
                  <UploadBox onClick={() => additionalImagesRef.current?.click()}>
                    <CloudUploadIcon sx={{ fontSize: 48, color: 'primary.main', mb: 2 }} />
                    <Typography variant="h6" color="primary" sx={{ mb: 1 }}>
            ...
            <Card sx={{ mb: 3 }}>
              ...
              <CardContent sx={{ p: 3 }}>
                <Stack spacing={2}>
                  ...
                  />
                  {errors.instructionLeaflet && (
                    <Alert severity="error">{errors.instructionLeaflet}</Alert>
                  )}
                  {instructionLeaflet && (
                    <Box>
                      <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 600 }}>
                      ...
                      >
                        <Typography variant="body2">{instructionLeaflet.name}</Typography>
                        <IconButton
                          size="small"
            ...
            <Card sx={{ mb: 3 }}>
              ...
              <CardContent sx={{ p: 3 }}>
                <Stack spacing={2}>
                  ...
                  />
                  {errors.prescriptionSample && (
                    <Alert severity="error">{errors.prescriptionSample}</Alert>
                  )}
                  {prescriptionSample && (
                    <Box>
                      <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 600 }}>
                      ...
                      >
                        <img
                          src={URL.createObjectURL(prescriptionSample)}
        ...
        <ParentCard title="Lifecycle & Availability" sx={{ mt: 3 }}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <CustomFormLabel htmlFor="zoneId" required>
                Select Zone
              </CustomFormLabel>
              <TextField
                select
              ...
              >
                {formData.metaKeywords.map((keyword, index) => (
                  <Chip
                    key={index}
                    label={keyword}
                    onDelete={() => removeKeyword(index, 'metaKeywords')}
                    sx={{ mr: 0.5 }}
                ...
                <CustomTextField
                  variant="standard"
                  onChange={(e) => handleKeywordChange(e, 'metaKeywords')}
                  onKeyDown={(e) => handleKeywordKeyDown(e, 'metaKeywords')}
        ...
        >
          ...
          <LoadingButton
            ...
            type="submit"
            sx={{ px: 4, py: 1.5 }}
            aria-label="Add medicine product"
          >
...
export default AddMedicineProduct;
