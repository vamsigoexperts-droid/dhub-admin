...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Weight' }];
...
function Brands() {
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.AddWeight, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    axios
      .put(`${URLS.EditWeight}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      axios
        .delete(`${URLS.DeleteWeight}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast(res.data.message);
  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => setData(res.data.weights || []))
      .catch((err) => toast('Failed to fetch Weight'))
  ...
  const columns = [
    ...
    { field: 'name', headerName: 'Weight', flex: 1 },
    { field: 'status', headerName: 'Status', flex: 1 },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" gap={1}>
          {rolesAndPermission.medicine_weight_edit === true ||
          ...
          {rolesAndPermission.medicine_weight_delete === true ||
          rolesAndPermission.accessAll === true ? (
            <>
              <Button
                size="small"
  ...
  return (
    <PageContainer title="Weight" description="this is Weight page">
      <Breadcrumb title="Weight" items={BCrumb} />
      {showAdd && (
        ...
        <CardHeader
          ...
          action={
            <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
              ...
              />{' '}
              {rolesAndPermission.medicine_weight_add === true ||
              rolesAndPermission.accessAll === true ? (
                <>
