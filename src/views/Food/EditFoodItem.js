...
const EditDish = () => {
  ...
  const [initialLoading, setInitialLoading] = useState(!!foodId);
  ...
  const validateForm = useCallback(() => {
    ...
    // Cuisine Type
    if (!formData.cuisinetypeId || formData.cuisinetypeId.length === 0) {
      newErrors.cuisinetypeId = 'At least one cuisine type is required';
    }
    ...
    // Subcategory (required if category is selected and subcategories exist)
    ...
    } else if (formData.shortDescription.length < 1 || formData.shortDescription.length > 200) {
      newErrors.shortDescription = 'Short description must be between 100 and 200 characters';
    }
    ...
    // Item Type (default 'Veg' ensures this is always valid, but check for safety)
    ...
    if (images.length === 0 && imagePreviews.length === 0) {
      newErrors.dishImages = 'At least one dish image is required';
    }

    // Stock Status (default 'inStock' ensures this is valid, but check for safety)
    ...
    // Packaging Charge
    if (
      formData.packagingCharge &&
      (isNaN(formData.packagingCharge) || formData.packagingCharge < 0)
    ) {
      newErrors.packagingCharge = 'Packaging charge must be a non-negative number';
    }

    // Delivery Eligibility (boolean, always valid via switch, no validation needed)

    // Out-of-Stock Alert Threshold
    if (
      formData.outOfStockAlertThreshold &&
      (isNaN(formData.outOfStockAlertThreshold) || formData.outOfStockAlertThreshold < 0)
    ) {
      newErrors.outOfStockAlertThreshold =
        'Out-of-stock alert threshold must be a non-negative number';
    }

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  }, [formData, images, imagePreviews, subcategories]);
  ...
  const fetchExistingData = useCallback(async () => {
    ...
    try {
      ...
      setFormData({
        ...data,
        cuisinetypeId: Array.isArray(data.cuisinetypeId) ? data.cuisinetypeId : [],
        zoneId: Array.isArray(data.zoneId) ? data.zoneId : [],
        dietaryLabels: Array.isArray(data.dietaryLabels) ? data.dietaryLabels : [],
        ...
        variantsCustomizations:
          Array.isArray(data.variantsCustomizations) && data.variantsCustomizations.length > 0
            ? data.variantsCustomizations
            : [{ name: '', basePrice: '', offerPriceDiscount: '', taxRate: '' }],
        addOnGroups:
          Array.isArray(data.addOnGroups) && data.addOnGroups.length > 0
            ? data.addOnGroups
            : [{ name: '', price: '' }],
        availabilitySchedule:
          Array.isArray(data.availabilitySchedule) && data.availabilitySchedule.length > 0
            ? data.availabilitySchedule
            : [{ day: '', fromTime: '', toTime: '' }],
      });

      if (data.dishImages) {
        const previews = Array.isArray(data.dishImages)
          ? data.dishImages.map((img) => `${URLS.FileBase}${img}`)
          : [];
      ...
      if (data.video) {
        setVideoPreview(`${URLS.FileBase}${data.video}`);
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to fetch dish data');
    } finally {
      setInitialLoading(false);
    }
  }, [foodId, token]);
  ...
  const handleImageUpload = useCallback(
    (e) => {
      ...
      setErrors((prev) => ({ ...prev, dishImages: '' }));
    },
  ...
  const removeImage = useCallback(
    (idx) => {
      ...
      if (images.length - 1 === 0 && imagePreviews.length - 1 === 0) {
        setErrors((prev) => ({ ...prev, dishImages: 'At least one dish image is required' }));
      }
    },
    [images.length, imagePreviews.length],
  );
...
const handleKeywordKeyDown = useCallback((e) => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      // Existing validation for basePrice
      if (!formData.basePrice) {
        toast.error('Base price is required');
        setErrors((prev) => ({ ...prev, basePrice: 'Base price is required' }));
        return;
      }
      ...
      try {
        ...
        const response = await axios.put(`${URLS.EditFoodItem}${foodId}`, fd, {
        ...
        let errorMessage = 'Failed to update dish. Please try again.';
    ...
    [formData, images, thumbnail, video, token, navigate, validateForm],
  );
  ...
  const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Edit Dish' }];
  ...
  return (
    <PageContainer title="Edit Dish">
      <Breadcrumb title="Edit Dish" items={BCrumb} />
        ...
        <ParentCard title="Basic Dish Information" sx={{ mt: 3 }}>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              ...
              />
              {errors.dishName && <FormHelperText error>{errors.dishName}</FormHelperText>}
            </Grid>
            <Grid item xs={12} md={6}>
            ...
            <Grid item xs={12} md={6}>
              ...
              {errors.cuisinetypeId && (
                <FormHelperText error>{errors.cuisinetypeId}</FormHelperText>
              )}
            </Grid>
            <Grid item xs={12} md={6}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <CustomTextField
                ...
                onChange={handleChange}
                placeholder="Brief overview (100â€“200 characters)"
                inputProps={{ maxLength: 200, 'aria-label': 'Short Description' }}
                required
                error={!!errors.shortDescription}
              />
              {errors.shortDescription && (
                <FormHelperText error>{errors.shortDescription}</FormHelperText>
              )}
            </Grid>
            <Grid item xs={12}>
              ...
              {errors.fullDescription && (
                <FormHelperText error>{errors.fullDescription}</FormHelperText>
              )}
            </Grid>
            <Grid item xs={12} md={6}>
              <FormControlLabel
            ...
            <Card sx={{ mb: 3 }}>
              ...
              <CardContent sx={{ p: 3 }}>
                <Stack spacing={2}>
                  ...
                  />
                  {errors.dishImages && <FormHelperText error>{errors.dishImages}</FormHelperText>}
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6} md={3}>
              <CustomFormLabel htmlFor="basePrice">
                Base Price (â‚¹) <span style={{ color: 'red' }}>*</span>
              </CustomFormLabel>
              <CustomTextField
                fullWidth
                type="number"
                ...
                onChange={handleChange}
                placeholder="0.00"
                inputProps={{ min: 0, step: 0.01, 'aria-label': 'Base Price', required: true }}
                required
                error={!!errors.basePrice}
              />
              {errors.basePrice && <FormHelperText error>{errors.basePrice}</FormHelperText>}
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <CustomFormLabel htmlFor="offerPriceDiscount">
                Offer Price (â‚¹) <span style={{ color: 'red' }}>*</span>
              </CustomFormLabel>
              <CustomTextField
                required
                fullWidth
        ...
        <ParentCard title="Inventory & Availability" sx={{ mt: 3 }}>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              ...
              {errors.stockStatus && <FormHelperText error>{errors.stockStatus}</FormHelperText>}
            </Grid>
            <Grid item xs={12} md={6}>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <CustomFormLabel htmlFor="packagingCharge">
                Packaging Charge (â‚¹) <span style={{ color: 'red' }}>*</span>
              </CustomFormLabel>
              <CustomTextField
                fullWidth
                type="number"
                ...
                onChange={handleChange}
                placeholder="0.00"
                inputProps={{ min: 0, step: 0.01, 'aria-label': 'Packaging Charge' }}
                error={!!errors.packagingCharge}
              />
              {errors.packagingCharge && (
                <FormHelperText error>{errors.packagingCharge}</FormHelperText>
              )}
            </Grid>
            <Grid item xs={12} md={6}>
              <FormControlLabel
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              ...
              <CustomTextField
                ...
                onChange={handleChange}
                placeholder="E.g., 10"
                inputProps={{ min: 0, 'aria-label': 'Out-of-Stock Alert Threshold' }}
                error={!!errors.outOfStockAlertThreshold}
              />
              {errors.outOfStockAlertThreshold && (
                <FormHelperText error>{errors.outOfStockAlertThreshold}</FormHelperText>
              )}
            </Grid>
            <Grid item xs={12}>
