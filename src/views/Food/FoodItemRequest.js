...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Food Request' }];
...
const CategoriesRequest = () => {
    ...
    if (window.confirm(`Are you sure you want to approve "${categoryData.name}" category?`)) {
      ...
      try {
        const res = await axios.put(
          `${URLS.UpdateFoodItemRequest}/${categoryData.productId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await axios.put(
        `${URLS.UpdateFoodItemRequest}/${selectedCategory.productId}`,
        {
    ...
    try {
      ...
      if (res.data.success && Array.isArray(res.data.data)) {
        const mappedData = res.data.data.map((item) => ({
          _id: item._id,
          name: item.dishName || 'Unnamed Product',
        ...
        renderCell: (params) => {
          return (
            <Box display="flex" gap={1}>
              {rolesAndPermission.food_items_request_edit === true ||
              ...
              {rolesAndPermission.food_items_request_delete === true ||
              rolesAndPermission.accessAll === true ? (
                <>
                  <Button
                    size="small"
  ...
  return (
    <PageContainer
      title="Food Request"
      description="Review and manage Food Request for your e-commerce platform"
    >
      <Breadcrumb title="Food Request" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Food Request List</Typography>
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              aria-label="Search Food Request"
            />
          </Box>
        </Box>
        <Divider />
