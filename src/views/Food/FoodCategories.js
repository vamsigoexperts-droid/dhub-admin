...
const changeHandler = (e) => {
  ...
  if (selectedFile) {
    ...
    // Accept ALL image types + ZIP
    const allowedExtensions = [
      'jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 
      'tiff', 'tif', 'heic', 'jfif', 'ico', 'avif',
      'zip'
    ];
    ...
    if (allowedExtensions.includes(ext)) {
      ...
      // Preview ONLY for image types (not ZIP)
      if (ext !== 'zip') {
        setPreview(URL.createObjectURL(selectedFile));
      } else {
        setPreview(null); // ZIP cannot be previewed
      }
...
const EditCategoryForm = ({ onClose, onSubmit, initialData, loading }) => {
  ...
  // Accept all image types for main image
  const changeHandler = (e) => {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
      setFile(selectedFile);
  ...
  // Accept all image types for category icon image
  const iconChangeHandler = (e) => {
    const selectedFile = e.target.files?.[0];
    if (!selectedFile) return;
    setIconFile(selectedFile);
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.food_categories_edit === true ||
            ...
            {rolesAndPermission.food_categories_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            />
            {rolesAndPermission.food_categories_add === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  variant="contained"
