...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Make' }];
...
function Make() {
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.AddMake, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    axios
      .put(`${URLS.EditMake}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      axios
        .delete(`${URLS.DeleteMake}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast(res.data.message);
  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => setData(res.data.vechile || []))
      .catch((err) => toast('Failed to fetch Make'))
  ...
  return (
    <PageContainer title="Make" description="this is Make page">
      <Breadcrumb title="Make" items={BCrumb} />
      {/* Add Section */}
      {showAdd && (
