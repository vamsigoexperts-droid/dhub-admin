...
const Cancelled = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(`Failed to fetch cancelled ${pageTitle.toLowerCase()}`);
            }
        } catch (error) {
            console.error('Error fetching orders:', error);
            toast.error(error.response?.data?.message || `Failed to fetch cancelled ${pageTitle.toLowerCase()}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleFilterChange = (newFilters) => {
        setFilters({ ...newFilters, status: 'cancelled' });
        setPage(0);
    };
    ...
    return (
        <PageContainer title={`Cancelled ${pageTitle}`} description={`Cancelled ${pageTitle.toLowerCase()}`}>
            <Breadcrumb title={`Cancelled ${pageTitle}`} items={BCrumb} />
