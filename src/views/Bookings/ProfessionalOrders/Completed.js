...
const Completed = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(`Failed to fetch completed ${pageTitle.toLowerCase()}`);
            }
        } catch (error) {
            console.error('Error fetching orders:', error);
            toast.error(error.response?.data?.message || `Failed to fetch completed ${pageTitle.toLowerCase()}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <PageContainer title={`Completed ${pageTitle}`} description={`Completed ${pageTitle.toLowerCase()}`}>
            <Breadcrumb title={`Completed ${pageTitle}`} items={BCrumb} />
