...
const InProgress = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(`Failed to fetch in-progress ${pageTitle.toLowerCase()}`);
            }
        } catch (error) {
            console.error('Error fetching orders:', error);
            toast.error(error.response?.data?.message || `Failed to fetch in-progress ${pageTitle.toLowerCase()}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleFilterChange = (newFilters) => {
        setFilters({ ...newFilters, status: 'in-progress' });
        setPage(0);
    };
    ...
    return (
        <PageContainer title={`In Progress ${pageTitle}`} description={`In progress ${pageTitle.toLowerCase()}`}>
            <Breadcrumb title={`In Progress ${pageTitle}`} items={BCrumb} />
