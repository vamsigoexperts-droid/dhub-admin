...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Faqs' }];
...
function Faqs() {
  ...
  const [form, setForm] = useState({ question: '', answer: '', type: 'customer' });
  const [formEdit, setFormEdit] = useState({ question: '', answer: '', _id: '', type: 'customer' });
  ...
  const handleInputChange = (e) => {
    setForm({
      ...
      type: faqTypes[currentTab], // Set type based on current tab
    });
  };
  ...
  const handleAddSubmit = (e) => {
    ...
    setLoading(true);
    axios
      .post(URLS.AddTypeFaqs, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    setLoading(true);
    axios
      .put(`${URLS.EditTypeFaqs}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete this FAQ?')) {
      setLoading(true);
      axios
        .delete(`${URLS.DeleteTypeFaqs}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
  ...
  const getData = (type = faqTypes[currentTab]) => {
    setLoading(true);
    axios
      .get(URLS.GetTypeFaqs + type, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  return (
    <PageContainer title="Faqs" description="this is Faqs page">
      <Breadcrumb title="Faqs" items={BCrumb} />
      ...
      {showEdit && (
        <form onSubmit={handleEditSubmit}>
          <ParentCard title="Edit FAQ" sx={{ overflowX: 'hidden' }}>
            <Grid container rowSpacing={3}>
              <Grid item xs={12} sx={{ padding: 1 }}>
