...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Shipping Policy' }];
...
const Terms = () => {
  ...
  const getData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || `Failed to fetch ${tabLabels[activeTab]} Shipping Policy`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      toast.error('Failed to fetch Shipping Policy data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      toast.success(`${tabLabels[activeTab]} Shipping Policy updated successfully!`);
    } catch (error) {
      toast.error(
        error.response?.data?.message || `Failed to update ${tabLabels[activeTab]} Shipping Policy`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      toast.success('All Shipping Policy updated successfully!');
    } catch (error) {
      toast.error('Failed to update some Shipping Policy');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageContainer title="Shipping Policy" description="This is Shipping Policy page">
      <Breadcrumb title="Shipping Policy" items={BCrumb} />
      <ParentCard title="Shipping Policy Management">
        {/* Tabs */}
          ...
          <TabPanel key={type} value={activeTab} index={index}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" gutterBottom>
                {tabLabels[index]} App Shipping Policy
              </Typography>
            ...
            {loading ? (
              ...
              <CKEditor
                ...
                config={{
                  ...
                  placeholder: `Type ${tabLabels[index]} Shipping Policy here...`,
                }}
                disabled={saving}
              />
            )}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, mt: 2 }}>
          {rolesAndPermission.shipping_policy_edit === true ||
