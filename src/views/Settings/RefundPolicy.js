...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Refund Policy' }];
...
const RefundPolicy = () => {
  ...
  const getData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || `Failed to fetch ${tabLabels[activeTab]} Refund Policy`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      toast.success(`${tabLabels[activeTab]} Refund Policy updated successfully!`);
    } catch (error) {
      toast.error(
        error.response?.data?.message || `Failed to update ${tabLabels[activeTab]} Refund Policy`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      toast.success('All Refund Policy updated successfully!');
    } catch (error) {
      toast.error('Failed to update some Refund Policy');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageContainer title="Refund Policy" description="This is Refund Policy page">
      ...
      <ParentCard title="Refund Policy Management">
          ...
          <TabPanel key={type} value={activeTab} index={index}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" gutterBottom>
                {tabLabels[index]} App Refund Policy
              </Typography>
            ...
            {loading ? (
              ...
              <CKEditor
                ...
                config={{
                  ...
                  placeholder: `Type ${tabLabels[index]} Refund Policy here...`,
                }}
                disabled={saving}
              />
            )}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, mt: 2 }}>
          {rolesAndPermission.refund_policy_edit === true ||
