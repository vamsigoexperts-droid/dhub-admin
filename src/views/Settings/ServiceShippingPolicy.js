...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Service Shipping Policy' }];
...
const Terms = () => {
  ...
  const getData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      const shippingContent =
        res.data.policy?.shippingPolicy?.paragraph || '<p>No content available</p>';
      ...
      toast.error(
        error.response?.data?.message ||
          `Failed to fetch ${tabLabels[activeTab]} Service Shipping Policy`,
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          const shippingContent =
            result.value.data.policy?.shippingPolicy?.paragraph || '<p>No content available</p>';
          newData[type] = shippingContent;
        } else {
      ...
      toast.error('Failed to fetch Service Shipping Policy data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      await axios.put(
        ...
        { shippingPolicy: currentData },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      toast.success(`${tabLabels[activeTab]} Service Shipping Policy updated successfully!`);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          `Failed to update ${tabLabels[activeTab]} Service Shipping Policy`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      toast.success('All Service Shipping Policy updated successfully!');
    } catch (error) {
      toast.error('Failed to update some Service Shipping Policy');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageContainer
      ...
      description="This is Service Shipping Policy page"
    >
      <Breadcrumb title="Service Shipping Policy" items={BCrumb} />
      <ParentCard title="Service Shipping Policy Management">
        {/* Tabs */}
          ...
          <TabPanel key={type} value={activeTab} index={index}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" gutterBottom>
                {tabLabels[index]} Service Shipping Policy
              </Typography>
            ...
            {loading ? (
              ...
              <CKEditor
                ...
                config={{
                  ...
                  placeholder: `Type ${tabLabels[index]} Service Shipping Policy here...`,
                }}
                disabled={saving}
              />
            )}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, mt: 2 }}>
          {rolesAndPermission.service_shipping_policy_edit === true ||
