...
import { Button, Box, Tabs, Tab, Typography, CircularProgress, Paper, Grid } from '@mui/material';
import { URLS } from 'src/Url';
import axios from 'axios';

const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Service Terms & Conditions' }];
...
const Terms = () => {
  ...
  const getData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      const termsContent =
        res.data.policy?.termsAndConditions?.paragraph || '<p>No content available</p>';
      ...
      toast.error(
        error.response?.data?.message ||
          `Failed to fetch ${tabLabels[activeTab]} Service Terms & Conditions`,
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          const termsContent =
            result.value.data.policy?.termsAndConditions?.paragraph ||
            '<p>No content available</p>';
          newData[type] = termsContent;
        } else {
      ...
      toast.error('Failed to fetch Service Terms & Conditions data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      await axios.put(
        ...
        { termsAndConditions: currentData },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      toast.success(`${tabLabels[activeTab]} Service Terms & Conditions updated successfully!`);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          `Failed to update ${tabLabels[activeTab]} Service Terms & Conditions`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      toast.success('All Service Terms & Conditions updated successfully!');
    } catch (error) {
      toast.error('Failed to update some Service Terms & Conditions');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageContainer
      ...
      description="This is Service Terms & Conditions page"
    >
      <Breadcrumb title="Service Terms & Conditions" items={BCrumb} />
      <ParentCard title="Service Terms & Conditions Management">
        {/* Tabs */}
          ...
          <TabPanel key={type} value={activeTab} index={index}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" gutterBottom>
                {tabLabels[index]} Service Terms & Conditions
              </Typography>
            ...
            {loading ? (
              ...
              <CKEditor
                ...
                config={{
                  ...
                  placeholder: `Type ${tabLabels[index]} Service Terms & Conditions here...`,
                }}
                disabled={saving}
              />
            )}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, mt: 2 }}>
          {rolesAndPermission.service_terms_conditions_edit === true ||
