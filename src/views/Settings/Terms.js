...
const Terms = () => {
  ...
  const getCurrentData = () => data[getCurrentType()];
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          `Failed to fetch ${tabLabels[activeTab]} Terms & Conditions`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const getAllData = async () => {
    ...
    try {
      ...
      toast.error('Failed to fetch Terms & Conditions data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateData = async (type = getCurrentType()) => {
    ...
    try {
      ...
      toast.success(`${tabLabels[activeTab]} Terms & Conditions updated successfully!`);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          `Failed to update ${tabLabels[activeTab]} Terms & Conditions`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      toast.success('All Terms & Conditions updated successfully!');
    } catch (error) {
      toast.error('Failed to update some Terms & Conditions');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageContainer title="Terms & Conditions" description="This is Terms & Conditions page">
      <Breadcrumb title="Terms & Conditions" items={BCrumb} />
      <ParentCard title="Terms & Conditions Management">
        {/* Tabs */}
          ...
          <TabPanel key={type} value={activeTab} index={index}>
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" gutterBottom>
                {tabLabels[index]} App Terms & Conditions
              </Typography>
            ...
            {loading ? (
              ...
              <CKEditor
                ...
                config={{
                  ...
                  placeholder: `Type ${tabLabels[index]} Terms & Conditions here...`,
                }}
                disabled={saving}
              />
            )}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, mt: 2 }}>
          {rolesAndPermission.terms_conditions_edit === true ||
