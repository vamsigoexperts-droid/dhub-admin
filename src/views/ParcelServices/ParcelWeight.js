...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Parcel Weight' }];
...
function ParcelWeight() {
  ...
  const [form, setForm] = useState({ name: '', deliveryCharge: '' });
  const [formEdit, setFormEdit] = useState({ name: '', _id: '', deliveryCharge: '' });
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.AddParcelWeight, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    const formData = {
      name: formEdit.name,
      deliveryCharge: formEdit.deliveryCharge,
    };

    axios
      .put(`${URLS.EditParcelWeight}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      axios
        .delete(`${URLS.DeleteParcelWeight}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast(res.data.message);
  ...
  const getData = () => {
    setLoading(true);
    axios
      .post(URLS.GetParcelWeight, {}, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => setData(res.data.parcelweight || []))
      .catch((err) => toast('Failed to fetch  Parcel weight'))
      .finally(() => setLoading(false));
  };

  const resetForm = () => {
    setForm({ name: '', deliveryCharge: '' });
    setFormEdit({ name: '', _id: '', deliveryCharge: '' });
    setShowAdd(false);
    setShowEdit(false);
  };
  ...
  const columns = [
    ...
    { field: 'deliveryCharge', headerName: 'Delivery Charge', flex: 1 },
  ...
  return (
    <PageContainer title="Parcel Weight" description="this is Parcel Weight page">
      <Breadcrumb title="Parcel Weight" items={BCrumb} />
      {/* Add Section */}
      {showAdd && (
