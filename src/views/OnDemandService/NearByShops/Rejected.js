...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Rejected Orders' }];
...
// Main Rejected Orders Component
const RejectedOrders = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.bookings) {
        const transformedData = response.data.bookings.map((booking) => ({
          ...
          rejectedAt: booking.logModifiedDate
            ...
            : 'N/A',
          rejectionReason:
            booking.rejectedReason || booking.statusTextMessage || 'Partner unavailable',
          alternativePartnersAvailable:
            booking.alternativePartnersAvailable || Math.floor(Math.random() * 6) + 1, // Random for demo
        ...
        toast.error('Failed to fetch rejected orders data');
        setBookingsData([]);
      }
    } catch (error) {
      console.error('Error fetching rejected orders:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch rejected orders');
      setBookingsData([]);
    } finally {
      setLoading(false);
    }
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          >
            ...
            >
              Reason: {params.row.rejectionReason?.substring(0, 25)}...
            </Typography>
          </Box>
        ),
      },
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          >
            {getAlternativePartnersDisplay(params.row.alternativePartnersAvailable)}
          </Box>
        ),
      },
      {
  ...
  return (
    <PageContainer
      ...
      description="Manage orders rejected by partners and handle reassignments"
    >
      <Breadcrumb title="Rejected Orders" items={BCrumb} />
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Orders rejected by partners requiring reassignment to alternative providers (
        ...
        <form onSubmit={handleEditSubmit}>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12}>
                ...
                >
                  ...
                  <MenuItem value="reassign-auto">Auto-assign to Available Partner</MenuItem>
                  <MenuItem value="reassign-manual">Manually Select Partner</MenuItem>
                  <MenuItem value="broadcast-again">Broadcast to All Partners</MenuItem>
                  <MenuItem value="cancel-order">Cancel Order (No Partners Available)</MenuItem>
                  <MenuItem value="escalate">Escalate to Management</MenuItem>
                </CustomSelect>
              </Grid>
              <Grid item xs={12}>
                ...
                <CustomTextField
                  ...
                  onChange={handleEditInputChange}
                  placeholder="Add reassignment notes or escalation reasons..."
                  multiline
                  rows={3}
                  fullWidth
                />
          ...
          <DialogActions sx={{ p: 3 }}>
            ...
            <Button
              ...
              startIcon={loading ? <CircularProgress size={16} /> : null}
            >
              {loading ? 'Updating...' : 'Execute Action'}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
