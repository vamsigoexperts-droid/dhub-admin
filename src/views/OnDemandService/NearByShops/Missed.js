...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Missed Orders' }];
...
const MissedOrders = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.bookings) {
        const transformedData = response.data.bookings.map((booking) => ({
          ...
          missedAt: booking.logModifiedDate
          ...
          missedReason:
            booking.statusTextMessage || 'No partner response within allocated time window',
          availablePartnersNotified:
            booking.availablePartnersNotified || Math.floor(Math.random() * 10) + 5, // Random for demo
          autoReassignmentAttempts:
            booking.autoReassignmentAttempts || Math.floor(Math.random() * 3) + 1, // Random for demo
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Order recovery action executed successfully');
      handleCloseModal();
      fetchBookings();
    } catch (error) {
    ...
    () => [
          ...
          >
            <Box display="flex" alignItems="center" gap={0.5} sx={{ mb: 0.5 }}>
              <Avatar
                ...
                sx={{ width: 22, height: 22, fontSize: '0.7rem', opacity: 0.5 }}
              >
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          >
            ...
            >
              Timeout: {params.row.timeoutDuration}
            </Typography>
          </Box>
        ),
      },
        ...
        renderCell: (params) => (
          ...
          >
            <Typography
              variant="caption"
              color="text.secondary"
              sx={{ fontSize: '0.65rem', lineHeight: 1.2, mb: 0.2 }}
            >
              {params.row.availablePartnersNotified} notified
            </Typography>
            <Typography
              variant="caption"
              color="warning.main"
              sx={{ fontSize: '0.65rem', lineHeight: 1.2 }}
            >
              {params.row.autoReassignmentAttempts} attempts
            </Typography>
          </Box>
        ),
      },
      {
        ...
        renderCell: (params) => (
          ...
          >
            {getRecoveryStatusChip(params.row.recoveryStatus)}
          </Box>
        ),
      },
      {
  ...
  return (
    <PageContainer
      title="Missed Orders"
      description="Recover orders that weren't responded to within the allocated time"
    >
      <Breadcrumb title="Missed Orders" items={BCrumb} />
      ...
      >
        ...
        >
          <Box>
            <Typography variant="h6">Missed Orders</Typography>
            <Typography variant="body2" color="text.secondary">
              Orders that expired without partner response requiring immediate recovery action (
      ...
      <Dialog open={openModal} onClose={handleCloseModal} maxWidth="sm" fullWidth>
        <DialogTitle>Recover Missed Order</DialogTitle>
        <form onSubmit={handleEditSubmit}>
          <DialogContent>
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12}>
                <CustomFormLabel htmlFor="status">Recovery Action*</CustomFormLabel>
                <CustomSelect
                  id="status"
                  name="status"
                ...
                >
                  ...
                  <MenuItem value="emergency-broadcast">
                    Emergency Broadcast to All Partners
                  </MenuItem>
                  <MenuItem value="premium-assignment">Assign to Premium Partners</MenuItem>
                  <MenuItem value="extend-timeout">Extend Response Timeout</MenuItem>
                  <MenuItem value="customer-callback">Schedule Customer Callback</MenuItem>
                  <MenuItem value="cancel-with-compensation">
                    Cancel with Customer Compensation
                  </MenuItem>
                  <MenuItem value="escalate-management">Escalate to Senior Management</MenuItem>
                </CustomSelect>
              </Grid>
              <Grid item xs={12}>
                <CustomFormLabel htmlFor="comment">Recovery Notes</CustomFormLabel>
                <CustomTextField
                  id="comment"
                  name="comment"
                  value={formEdit.comment}
                  onChange={handleEditInputChange}
                  placeholder="Add recovery strategy details and customer communication notes..."
                  multiline
                  rows={3}
                  fullWidth
                />
