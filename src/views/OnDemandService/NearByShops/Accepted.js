...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Accepted Orders' }];
...
// Main Accepted Orders Component
const AcceptedOrders = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.bookings) {
        const transformedData = response.data.bookings.map((booking) => ({
          ...
          acceptedAt: booking.logModifiedDate
        ...
        toast.error('Failed to fetch accepted orders data');
        setBookingsData([]);
      }
    } catch (error) {
      console.error('Error fetching accepted orders:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch accepted orders');
      setBookingsData([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <PageContainer
      ...
      description="Manage orders accepted by partners ready for next action"
    >
      <Breadcrumb title="Accepted Orders" items={BCrumb} />
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Orders accepted by partners ready for next action ({filteredData.length} orders)
            </Typography>
          </Box>
        ...
        <form onSubmit={handleEditSubmit}>
          <DialogContent>
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12}>
                <CustomFormLabel htmlFor="status">Select the provider</CustomFormLabel>
                <CustomSelect
                  id="status"
                  name="status"
