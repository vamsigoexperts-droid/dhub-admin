...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Work In Progress' }];
...
// Main Work In Progress Orders Component
const WorkInProgress = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.bookings) {
        const transformedData = response.data.bookings.map((booking) => ({
          ...
          startedAt: booking.logModifiedDate
            ...
            : 'N/A',
          estimatedCompletion: booking.estimatedCompletion || 'To be updated',
          progressStatus: booking.progressStatus || 'In Progress',
        ...
        toast.error('Failed to fetch work in progress orders data');
        setBookingsData([]);
      }
    } catch (error) {
      console.error('Error fetching work in progress orders:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch work in progress orders');
      setBookingsData([]);
    } finally {
      setLoading(false);
    }
      ...
      },
      {
        ...
        renderCell: (params) => (
          ...
          >
            ...
            {getProgressChip(params.row.progressStatus)}
          </Box>
        ),
      },
      {
  ...
  return (
    <PageContainer
      ...
      description="Monitor and manage ongoing service deliveries"
    >
      <Breadcrumb title="Work In Progress" items={BCrumb} />
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Services currently being performed by partners ({filteredData.length} orders)
            </Typography>
          </Box>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12}>
                ...
                <CustomTextField
                  ...
                  onChange={handleEditInputChange}
                  placeholder="Add progress update or completion notes..."
                  multiline
                  rows={3}
                  fullWidth
                />
