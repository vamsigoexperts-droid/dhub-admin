...
const SubCategories = () => {
  ...
  const handleSubmit = async (formData, id) => {
    ...
    try {
      ...
      let res;
      if (id) {
        res = await axios.put(`${URLS.UpdateSellSubCategories}/${id}`, formData, config);
      } else {
        res = await axios.post(URLS.AddSellSubCategories, formData, config);
      }
      if (res.status === 200) {
  ...
  const handleDelete = async (data) => {
    ...
    if (window.confirm('Do you really want to delete this category?')) {
      setLoading(true);
      try {
        const res = await axios.delete(`${URLS.DeleteSellSubCategories}/${data._id}`, {
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.buy_sell_subcategory_edit === true ||
            ...
            {rolesAndPermission.buy_sell_subcategory_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
