...
const AcceptedOrders = () => {
    ...
    try {
      ...
      if (allBookings.length > 0) {
        const transformedData = allBookings.map((booking) => ({
          ...
          location: `${booking.addressArea || ''}, ${booking.addressCityName || ''}, ${booking.addressStateName || ''
            }`.replace(/^,\s*|,\s*$/g, ''),
  ...
  // FIXED: Navigate to Assign Provider screen with address details
  ...
  const handleSendToServiceProvider = () => {
    ...
    // Find details for the first selected order to pass address info
    // (Limitation: specific to single selection or multiple from same area, but passing first as reference)
    const firstOrder = bookingsData.find(b => b._id === selectedOrders[0]);

    navigate('/verified-partners-crm/assign-providers', {
      state: {
        orderIds: selectedOrders,
        addressDetails: firstOrder?.addressDetails,
