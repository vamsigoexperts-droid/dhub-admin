...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Item Attribute' }];
...
const Itemattribute = () => {
  ...
  const handleSubmit = async (formData, id) => {
    ...
    try {
      ...
      const res = id
        ? await axios.put(`${URLS.EditItemAttribute}/${id}`, formData, config)
        : await axios.post(URLS.AddItemAttribute, formData, config);
  ...
  const handleDelete = async (row) => {
    ...
    setLoading(true);
    try {
      const res = await axios.delete(`${URLS.DeleteItemAttribute}/${row._id}`, {
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.post(
        URLS.GetItemAttribute,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      setData(res.data.itemattribute || []);
    } catch {
  ...
  return (
    <PageContainer title="Item Attribute" description="Manage your item attributes">
      <Breadcrumb title="Item Attribute" items={BCrumb} />
      ...
      <Paper variant="outlined" sx={{ p: 2, mt: 2, borderRadius: 2 }}>
        <Box display="flex" justifyContent="space-between" flexWrap="wrap" gap={2}>
          <Typography variant="h6">Item Attribute List</Typography>
