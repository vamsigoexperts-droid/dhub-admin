...
import { IconEye, IconAnalyze, IconTrash, IconLockOpen } from '@tabler/icons-react';
...
const BCrumb = [
  { to: '/', title: 'Home' },
  { title: 'Blocked Professional Providers' },
];

const BlockedProfessionalProviders = () => {
  ...
  // âœ… Fetch blocked professional provider data
  const getData = async (searchQuery = '') => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || 'Failed to fetch blocked providers.',
      );
  ...
  // âœ… Open unblock confirmation
  const handleUnblockConfirmation = (providerData) => {
  ...
  const handleUnblock = async () => {
    ...
    try {
      const url = `${URLS.Base}/v1/dhubApi/admin/professional-providers/update-professional-provider-status/${selectedProvider._id}`;
      ...
      toast.success(res.data.message || 'Provider unblocked successfully');
      getData(search.trim());
  ...
  const handleDelete = async (providerData) => {
    ...
    if (window.confirm('Do you really want to delete this blocked provider?')) {
      setLoading(true);
      try {
            ...
            <Box display="flex" alignItems="center" gap={2}>
              <Avatar
                ...
                sx={{
                  ...
                  opacity: 0.7, // Dimmed for blocked status
                }}
              >
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {(rolesAndPermission.professional_providers_edit === true ||
              ...
              <>
                <Tooltip title="Unblock Provider">
                  <Button
                    size="small"
                    color="success"
                    variant="contained"
                    onClick={() => handleUnblockConfirmation(params.row)}
                    disabled={loading}
                    sx={{ minWidth: '32px', padding: '4px 6px' }}
                  >
                    <IconLockOpen stroke={1.5} size={18} />
                  </Button>
                </Tooltip>
  ...
  return (
    <PageContainer
      title="Blocked Professional Providers"
      description="Manage blocked professional providers"
    >
      <Breadcrumb title="Blocked Professional Providers" items={BCrumb} />
      ...
      >
        ...
        >
          <Box display="flex" alignItems="center" gap={1}>
            <Typography variant="h6">Blocked Provider List</Typography>
        ...
        <CardContent>
          {data.length === 0 && !loading ? (
            <Alert severity="info" sx={{ mt: 2 }}>
              No blocked professional providers found.
            </Alert>
          ) : (
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Unblock Provider</DialogTitle>
        <DialogContent>
          <Alert severity="warning" sx={{ mt: 2 }}>
            Are you sure you want to unblock this provider? They will be able to access the
            platform again.
          </Alert>
          {selectedProvider && (
        ...
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={handleCloseUnblockModal} variant="outlined">
            Cancel
          </Button>
          <Button onClick={handleUnblock} variant="contained" color="success">
            Unblock
          </Button>
        </DialogActions>
      </Dialog>
...
export default BlockedProfessionalProviders;
