...
const BCrumb = [
  { to: '/', title: 'Home' },
  { title: 'Professional Provider Requests' },
];

const ProfessionalProviderPendingRequest = () => {
  ...
  // âœ… Fetch professional provider data
  const getData = async (searchQuery = '') => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || 'Failed to fetch professional providers.',
      );
  ...
  // âœ… Updated Delete provider with PUT request and reason
  ...
  return (
    <PageContainer
      title="Professional Provider Requests"
      description="Manage professional provider approval requests"
    >
      <Breadcrumb title="Professional Provider Pending Requests" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Professional Provider List ({data.length})</Typography>
      ...
      <Dialog open={deleteDialog} onClose={handleCloseDeleteDialog} maxWidth="sm" fullWidth>
        <DialogTitle>Delete Professional Provider</DialogTitle>
        <DialogContent>
          <Typography variant="body1" sx={{ mt: 2, mb: 2 }}>
            Are you sure you want to delete this provider?
          </Typography>

          <CustomFormLabel htmlFor="deleteReason">Deletion Reason* (Required)</CustomFormLabel>
          <CustomTextField
            id="deleteReason"
            name="deleteReason"
            value={deleteReason}
            onChange={(e) => setDeleteReason(e.target.value)}
            placeholder="Please provide reason for deletion (e.g., Violation of terms, Fake profile, etc.)"
            multiline
            rows={3}
            fullWidth
            required
...
export default ProfessionalProviderPendingRequest;
