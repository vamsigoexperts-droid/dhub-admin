...
const EditTestimonialsForm = ({ onClose, onSubmit, initialData }) => {
  ...
  const [preview, setPreview] = useState(
    initialData?.image ? getEncodedImageUrl(initialData.image) : null,  // âœ… Changed from profile_image to image
  );
        ...
        >
          ...
          >
            ...
            <Button
              ...
              variant="contained"
              type="submit"
              aria-label="Update Testimonials"
            >
              Submit
            </Button>
          </Box>
...
const Testimonials = () => {
  ...
  // âœ… DELETE CONFIRMATION DIALOG STATE
...
const handleSubmit = async (formData, id) => {
  ...
  try {
    ...
    let res;
    if (id) {
      res = await axios.put(`${URLS.EditTestimonials}/${id}`, formData, config);
    } else {
      res = await axios.post(URLS.AddTestimonials, formData, config);
    }

    if (res.status === 200 || res.status === 201) {
      // âœ… FIX 1: Close form first
      handleCloseForm();
      
      // âœ… FIX 2: Show toast AFTER closing (so it stays visible)
      toast.success(res.data.message || (id ? 'Testimonial updated successfully!' : 'Testimonial created successfully!'));
      
      // âœ… FIX 3: Refresh data
      await getData();
      
      // âœ… FIX 4: Scroll AFTER everything is done
      setTimeout(() => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await axios.delete(`${URLS.DeletTestimonials}/${deleteDialog.data._id}`, config);
  ...
  const getData = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const res = await axios.post(URLS.GetTestimonials, {}, config);
 ...
 const columns = useMemo(
  () => [
    ...
    },
    {
      ...
      minWidth: 200,
      renderCell: (params) => {
        // âœ… FIXED: Use 'image' field and getEncodedImageUrl helper
        const imageUrl = params.row.image 
          ? getEncodedImageUrl(params.row.image)
          : null;
        
        const firstLetter = params.row.name ? params.row.name[0].toUpperCase() : '?';
          ...
          <Box display="flex" alignItems="center" gap={2}>
            <Avatar
              ...
              sx={{
                width: 40,
                height: 40,
                bgcolor: !imageUrl ? theme.palette.primary.main : 'transparent',
              }}
              imgProps={{
                onError: (e) => {
    ...
    <PageContainer title="Testimonials" description="Manage Testimonials for your application">
      ...
      >
        ...
        <DialogContent sx={{ mt: 2 }}>
          ...
          >
            Are you sure you want to delete <strong>{deleteDialog.data?.name}</strong>? This action
            cannot be undone.
          </DialogContentText>
        </DialogContent>
