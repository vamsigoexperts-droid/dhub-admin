...
const Providers = () => {
  ...
  const [isLoading, setIsLoading] = useState({ mail: false });
  const [mailForm, setMailForm] = useState({ subject: '', messageHtml: '' });
  ...
  const handleEditorChange = (event, editor) => {
    ...
    setMailForm({ ...mailForm, messageHtml: data });
  };
  ...
  const handleMailSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        URLS.ProviderSendMail,
        { sub: mailForm.subject, body: mailForm.messageHtml, userId: selectedUser._id },
    ...
    } finally { setIsLoading((prev) => ({ ...prev, mail: false })); }
  };
  ...
  const handleBulkDelete = async () => {
    if (!token) return;
    if (selectionModel.length === 0) { toast.warning('Please select at least one provider.'); return; }
    const selectedProviders = selectionModel.map((id) => rows.find((r) => r.id === id)).filter(Boolean);
    if (!window.confirm(`Delete ${selectedProviders.length} provider(s)?`)) return;
    setLoading(true);
    try {
      for (const provider of selectedProviders) {
        await axios.delete(`${URLS.DeleteProvider}/${provider._id}`, { headers: { Authorization: `Bearer ${token}` } });
      }
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      for (const provider of selectedProviders) {
        const payload = { status: bulkForm.status, ...(bulkForm.status === 'rejected' && { comment: bulkForm.comment }) };
        await axios.put(`${URLS.UpdateProviderStatus}${provider._id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
      }
  ...
  const handleUpdatePopUp = (data) => {
    setFormEdit({ _id: data._id, status: data.status || '', comment: data.comment || '' });
    setOpenModal(true);
    handleActionMenuClose();
  };
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const payload = { status: formEdit.status, ...(formEdit.status === 'rejected' && { comment: formEdit.comment }) };
  ...
  const columns = useMemo(() => {
    const baseColumns = [
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" alignItems="center" height="100%">
            <Typography variant="body2">{params.row.childcategoryName}</Typography>
          </Box>
        ),
      },
      {
  ...
  return (
    <PageContainer title="Service Provider Page">
      ...
      >
        ...
        {(rolesAndPermission.all_providers_edit || rolesAndPermission.accessAll) && (
          <MenuItem onClick={() => handleMailClick(selectedRow)}>
            <ListItemIcon><IconMail size={18} color={theme.palette.warning.main} /></ListItemIcon>
      ...
      {/* Modals */}
      <Dialog open={openMailModal} onClose={handleMailModalClose} maxWidth="md" fullWidth>
        <DialogTitle>Send Email to {selectedUser?.firstName}</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <CustomFormLabel>Subject</CustomFormLabel>
              <TextField fullWidth value={mailForm.subject} onChange={(e) => setMailForm({ ...mailForm, subject: e.target.value })} />
            </Grid>
            <Grid item xs={12}>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleMailSubmit} disabled={isLoading.mail}>Send</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openBulkModal} onClose={() => setOpenBulkModal(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleBulkSubmit}>Update All</Button>
        </DialogActions>
      </Dialog>
    </PageContainer>
  );
