...
function SubShopVariation() {
  ...
  const isDuplicate = data.some(
    (item) =>
      ...
      item.shopVariationId === form.shopVariationId
  );

  if (isDuplicate) {
    toast.error('Sub-shop variation already exists');
    return;
  }

  axios
    .post(URLS.addSubShoppingVariation, form, {
...
const handleEditSubmit = (e) => {
  ...
  const isDuplicate = data.some(
    (item) =>
      ...
      item.shopVariationId === payload.shopVariationId
  );
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete this sub-Grocery variation ?')) {
      axios
  ...
  const fetchStates = () => {
    setLoading(true);
    axios                                    
      .post(URLS.getSubShoppingVariation, {flagType: 'grocery'}, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => setData(res.data.subVariations || []))
      .catch(() => toast.error('Failed to fetch sub-Grocery variations'))
