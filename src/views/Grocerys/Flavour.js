...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Flavour' }];
...
function Brands() {
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.AddFlavour, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    axios
      .put(`${URLS.EditFlavour}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      axios
        .delete(`${URLS.DeleteFlavour}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast(res.data.message);
  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => setData(res.data.flavours || []))
      .catch((err) => toast('Failed to fetch Flavour'))
  ...
  const columns = [
    ...
    { field: 'name', headerName: 'Flavour', flex: 1 },
    { field: 'status', headerName: 'Status', flex: 1 },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" gap={1}>
          {rolesAndPermission.grocery_flavour_edit === true ||
          ...
          {rolesAndPermission.grocery_flavour_delete === true ||
          rolesAndPermission.accessAll === true ? (
            <>
              <Button
                size="small"
  ...
  return (
    <PageContainer title="Flavour" description="this is Flavour page">
      <Breadcrumb title="Flavour" items={BCrumb} />
      {showAdd && (
        ...
        <CardHeader
          ...
          action={
            <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
              ...
              />{' '}
              {rolesAndPermission.grocery_flavour_add === true ||
              rolesAndPermission.accessAll === true ? (
                <>
