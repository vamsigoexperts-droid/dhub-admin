...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Grocery Variation' }];
...
function SizeType() {
  ...
  const handleAddSubmit = (e) => {
    ...
    axios
      .post(URLS.addShoppingVariation, formData, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    axios
      .put(`${URLS.editShoppingVariation}${formEdit._id}`, formData, {
  ...
  const handleDelete = (item) => {
    if (window.confirm('Do you really want to delete?')) {
      setLoading(true);
      axios
        .delete(`${URLS.deleteShoppingVariation}${item._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          toast.success(res.data.message);
  ...
  const getData = () => {
    ...
    axios
      ...
      .then((res) => setData(res.data.variations || []))
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" gap={1}>
          {rolesAndPermission.shop_variations_edit === true ||
          ...
          {rolesAndPermission.shop_variations_delete === true ||
          rolesAndPermission.accessAll === true ? (
            <>
              <Button
                size="small"
  ...
  return (
    <PageContainer title="Grocery Variation" description="This is Size Type page">
      <Breadcrumb title="Grocery Variation" items={BCrumb} />
      {showAdd && (
        ...
        <CardHeader
          ...
          action={
            <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
              ...
              {rolesAndPermission.shop_variations_add === true ||
              rolesAndPermission.accessAll === true ? (
                <>
                  <Button
                    variant="contained"
                    ...
                    disabled={loading}
                  >
                    <IconPlus /> Create Grocery Variation
                  </Button>
                </>
              ) : (
                <></>
