...
const AddCategoryForm = ({ onClose, onSubmit, loading }) => {
  ...
  const [iconFile, setIconFile] = useState(null);       // to store selected icon file
...
const iconChangeHandler = (e) => {
  ...
  setIconFile(selectedFile);                 // store the file
  setIconPreview(URL.createObjectURL(selectedFile)); // preview
};
...
const changeHandler = (e) => {
  ...
  if (selectedFile) {
    // Accept all image types by checking MIME type
    if (selectedFile.type.startsWith("image/")) {
      setFile(selectedFile);
      setPreview(URL.createObjectURL(selectedFile)); // preview any image
    } else {
      e.target.value = null;
...
const handleSubmit = (e) => {
  ...
  formData.append('flagType', 'grocery');

  // Append banner image
  formData.append('image', file);

  // Append category icon **if selected**
  if (iconFile) {
  ...
  />
  {iconPreview && <Avatar src={iconPreview} sx={{ width: 60, height: 60, mt: 1 }} />}
</Grid>
...
const changeHandler = (e) => {
  ...
  if (iconFile) {
    formData.append('categoryIcon', iconFile);  // append icon file
  }
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.grocery_categories_edit === true ||
            ...
            {rolesAndPermission.grocery_categories_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            />{' '}
            {rolesAndPermission.grocery_categories_add === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  variant="contained"
