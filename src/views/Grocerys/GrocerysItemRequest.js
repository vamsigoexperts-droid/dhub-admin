...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Groceries Request' }];
...
const CategoriesRequest = () => {
    ...
    if (window.confirm(`Are you sure you want to approve "${categoryData.name}" category?`)) {
      ...
      try {
        const res = await axios.put(
          `${URLS.UpdateGrocerysItemRequest}/${categoryData.productId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await axios.put(
        `${URLS.UpdateGrocerysItemRequest}/${selectedCategory.productId}`,
        {
  ...
  const handleDelete = async (data) => {
    ...
    if (window.confirm('Do you really want to delete this Product?')) {
      setLoading(true);
      try {
        const res = await axios.delete(`${URLS.DeleteGrocerysItem}/${data.productId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.status === 200) {
          toast.success(res.data.message);
        ...
        renderCell: (params) => {
          return (
            <Box display="flex" gap={1}>
              {rolesAndPermission.grocery_request_list_edit === true ||
              ...
              {rolesAndPermission.grocery_request_list_delete === true ||
              rolesAndPermission.accessAll === true ? (
                <>
                  <Button
                    size="small"
  ...
  return (
    <PageContainer
      title="Groceries Request"
      description="Review and manage Groceries Request for your e-commerce platform"
    >
      <Breadcrumb title="Groceries Request" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Groceries Request List</Typography>
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              aria-label="Search Groceries Request"
            />
          </Box>
        </Box>
        <Divider />
