...
const AddBrandForm = ({ onClose, onSubmit, serviceTypes }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!file) {
      toast.error('Image is required for new Brands.');
      return;
    }
  ...
  return (
    <Box sx={{ py: 1 }}>
      <form onSubmit={handleSubmit}>
        <ParentCard title="Create Brand" sx={{ boxShadow: theme.shadows[4], borderRadius: '8px' }}>
          ...
          >
            ...
            <Button color="primary" variant="contained" type="submit" aria-label="Create Brand">
              Submit
            </Button>
          </Box>
        </ParentCard>
...
const EditBrandForm = ({ onClose, onSubmit, initialData, serviceTypes }) => {
  const theme = useTheme();
  const [form, setForm] = useState({
  ...
  return (
    <Box sx={{ py: 2 }}>
      <form onSubmit={handleSubmit}>
        <ParentCard title="Edit Brand" sx={{ boxShadow: theme.shadows[4], borderRadius: '8px' }}>
          <Grid container spacing={2} sx={{ p: 2 }}>
            <Grid item xs={12} sm={4}>
              <CustomFormLabel htmlFor="name" required>
                Brand Name
          ...
          >
            ...
            <Button color="primary" variant="contained" type="submit" aria-label="Update Brand">
              Submit
            </Button>
          </Box>
        </ParentCard>
...
const Brands = () => {
  ...
  const handleSubmit = async (formData, id) => {
    ...
    try {
      ...
      let res;
      if (id) {
        res = await axios.put(`${URLS.EditBrand}/${id}`, formData, config);
      } else {
        res = await axios.post(URLS.AddBrand, formData, config);
      }
      if (res.status === 200) {
  ...
  const handleDelete = async (data) => {
    ...
    if (window.confirm('Do you really want to delete this Brand?')) {
      setLoading(true);
      try {
        const res = await axios.delete(`${URLS.DeleteBrand}/${data._id}`, {
  ...
  return (
    <PageContainer title="Brands" description="Manage Brands for your e-commerce platform">
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Brands List</Typography>
