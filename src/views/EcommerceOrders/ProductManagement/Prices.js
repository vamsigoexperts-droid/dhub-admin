...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Prices' }];
  ...
  return (
    <Box sx={{ py: 1 }}>
      <form onSubmit={handleSubmit}>
        <ParentCard title="Create Price" sx={{ boxShadow: theme.shadows[4], borderRadius: '8px' }}>
          ...
          >
            ...
            <Button color="primary" variant="contained" type="submit" aria-label="Create Price">
              Submit
            </Button>
          </Box>
        </ParentCard>
...
const EditPriceForm = ({
  ...
  return (
    <Box sx={{ py: 2 }}>
      <form onSubmit={handleSubmit}>
        <ParentCard title="Edit Price" sx={{ boxShadow: theme.shadows[4], borderRadius: '8px' }}>
          <Grid container spacing={2} sx={{ p: 2 }}>
            <Grid item xs={4}>
              <CustomFormLabel htmlFor="serviceId" required>
                Service Name
          ...
          >
            ...
            <Button color="primary" variant="contained" type="submit" aria-label="Update Price">
              Submit
            </Button>
          </Box>
        </ParentCard>
...
const Prices = () => {
  ...
  const handleSubmit = async (formData, id) => {
    ...
    try {
      ...
      let res;
      if (id) {
        res = await axios.put(`${URLS.UpdatePrice}/${id}`, formData, config);
      } else {
        res = await axios.post(URLS.AddPrice, formData, config);
      }
      if (res.status === 200) {
  ...
  const handleDelete = async (data) => {
    ...
    if (window.confirm('Do you really want to delete this Price?')) {
      setLoading(true);
      try {
        const res = await axios.delete(`${URLS.DeletePrice}/${data._id}`, {
  ...
  return (
    <PageContainer title="Prices" description="Manage Prices for your e-commerce platform">
      <Breadcrumb title="Prices" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Prices List</Typography>
