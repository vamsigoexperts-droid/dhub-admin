...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Prices Values' }];

// Add Prices Values Form Component
...
// Edit Prices Values Form Component
...
const AttributesValues = () => {
  ...
  const getData = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to fetch Prices Values';
      toast.error(errorMessage);
      console.error('Get Prices Values Error:', error);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        renderCell: (params) => {
          const attributeName =
            attributes.find((attr) => attr._id === params.row.attributeId)?.name || 'N/A';
  ...
  return (
    <PageContainer
      title="Prices Values"
      description="Manage Prices Values for your e-commerce platform"
    >
      <Breadcrumb title="Prices Values" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Prices Values</Typography>
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              disabled={loading}
              aria-label="Search Prices Values"
            />
            <Button
              variant="contained"
              color="primary"
