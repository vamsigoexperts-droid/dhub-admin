...
function EditZone() {
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          sessionStorage.setItem('tost', 'Zone has been Edited Successfully');
        }
      })
      .catch((error) => {
        if (error.response && error.response.status === 400) {
          ...
          toast.error('An error occurred while editing the zone');
        }
      })
      .finally(() => {
        setLoading(false);
  ...
  const GetOneZone = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setForm(zoneData);
        setNewZone(zoneData.coordinates);
        setNewZone1(zoneData.coordinates[0]);
        if (zoneData.countryId) {
          getStates(zoneData.countryId);
        }
        if (zoneData.stateId) {
          getCity(zoneData.stateId);
        }
      })
      .catch((error) => {
  ...
  return (
    <PageContainer title="Edit Zone" description="Edit zone">
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={2} sx={{ display: 'flex', alignItems: 'flex-end' }}>
                    <Button
                      ...
                      sx={{ height: '40px' }}
                    >
                      {loading ? <CircularProgress size={24} /> : 'Update Zone'}
                    </Button>
                  </Grid>
                </Grid>
              </form>
