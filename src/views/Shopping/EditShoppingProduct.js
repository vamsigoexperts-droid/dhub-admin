...
// Handle variations from fetched product
...
// Auto-generate permalink: subcategory â†’ productname
useEffect(() => {
  ...
  // Order: subcategory â†’ product ONLY
  const parts = [
...
// Auto-sync Tags/Keywords to SEO Keywords (lowercase)
useEffect(() => {
...
useEffect(() => {
  ...
  async (subcategoryId) => {
    ...
    if (childcategoriesCache[subcategoryId]) {
      setChildcategories(childcategoriesCache[subcategoryId]); // â† FIX: was setSubcategories
      return;
    }
      await fetchData(
...
const handleChange = useCallback(
  (e) => {
    ...
    setFormData((prev) => {
      ...
      // Discount calculation logic (keep your existing code)
  ...
  <Box sx={{ 
    ...
    {isEditingSlug ? (
      ...
      <Box sx={{
        ...
        >
          https://doorstephub.com/{formData.slug || 'subcategory-productname'}
        </Typography>
      </Box>
    )}
