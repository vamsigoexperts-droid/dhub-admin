...
import { IconButton, FormHelperText, CircularProgress } from '@mui/material';
...
// Auto-generate slug: category + subcategory
useEffect(() => {
    if (!slugManuallyEdited && form.name && form.categoryId) { 
    const categoryObj = serviceTypes.find(cat => cat._id === form.categoryId);
    const categorySlug = categoryObj?.slug || toSlug(categoryObj?.name || '');
    const subcategorySlug = toSlug(form.name);
    
    const generatedSlug = categorySlug && subcategorySlug 
      ? `${categorySlug}-${subcategorySlug}` 
      : subcategorySlug;
    
    setForm(prev => ({ ...prev, slug: generatedSlug }));
  }
}, [form.name, form.categoryId, serviceTypes, slugManuallyEdited]);
...
const handleSubmit = (e) => {
  ...
  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
    {isEditingSlug ? (
      <CustomTextField
        ...
        placeholder="https://doorstephub.com/category-subcategory"
        fullWidth
        autoFocus
      ...
      <Box sx={{
        ...
        >
          https://doorstephub.com/{form.slug || 'category-subcategory'}
        </Typography>
      </Box>
    )}
  ...
  <IconButton
    ...
    onClick={() => {
      if (isEditingSlug) {
        setSlugManuallyEdited(true); // â† FIX: Add this line
      }
...
const handleSubmit = (e) => {
  ...
  if (!form.slug) {  // â† ADD THIS VALIDATION
    toast.error('Permalink is required.');
    return;
  }
  const formData = new FormData();
  ...
  onClick={() => {
    if (isEditingSlug) {
      // Clicking checkmark - mark as manually edited permanently
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Box display="flex" gap={1}>
            {rolesAndPermission.shopping_subcategories_edit === true ||
            ...
            {rolesAndPermission.shopping_subcategories_delete === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  size="small"
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            />
            {rolesAndPermission.shopping_subcategories_add === true ||
            rolesAndPermission.accessAll === true ? (
              <>
                <Button
                  variant="contained"
