...
const getId = (obj) => {
  ...
  //********************************* testing data for the specifications *********************************************************
  const [variations, setVariations] = useState([]);
  const [subVariations, setSubVariations] = useState([]);
  //   const [loading, setLoading] = useState(true);
 // const [testformData, setTestformData] = useState({});
// Handler for variation and sub-variation selection
const handleVariationChange = (variationId) => (e) => {
  ...
  setFormData((prev) => {
    ...
    if (subVariationId) {
      // If a sub-variation is selected, update or add the variation
      ...
      // If sub-variation is cleared, remove the variation
  ...
  const validateForm = useCallback(() => {
    const newErrors = {};
    if (!mainImage) {
      newErrors.mainImage = 'At least one mainImage is required';
    }
  ...
  }, [formData, additionalImages, mainImage]);
  ...
  //*********************************************************** */  Fetch Variations for testing data
  ...
  // Auto-generate slug from category → subcategory → child → product
// Auto-generate slug: domain → productName → category → subcategory → child


// Auto-generate permalink: category → subcategory → child → productname
// Auto-generate permalink: subcategory → productname
useEffect(() => {
  // Skip if user is currently editing
  if (isEditingSlug) return;
  
  const subcategoryObj = subcategories.find(sc => getId(sc) === String(formData.subcategoryId));

  const subcategoryName = subcategoryObj?.name || "";
  const productName = formData.productName || "";

  // Order: subcategory → product ONLY
  const parts = [
    subcategoryName ? toSlug(subcategoryName) : null,
    productName ? toSlug(productName) : null,
  ].filter(Boolean);
...
// Auto-sync Tags/Keywords to SEO Title (lowercase)
useEffect(() => {
  if (formData.tagsKeywords.length > 0) {
    const lowercaseTags = formData.tagsKeywords.map(tag => tag.toLowerCase());
    setFormData(prev => ({ ...prev, metaKeywords: lowercaseTags }));
  }
}, [formData.tagsKeywords]);

// Auto-sync Short Description to SEO Description
useEffect(() => {
  if (formData.shortDescription) {
    setFormData(prev => ({ ...prev, metaDescription: formData.shortDescription }));
  }
...
const handleChange = useCallback((e) => {
  ...
  setFormData(prev => {
    ...
    if (name === "categoryId") {
      ...
      setSlugEdited(false); // Resume auto-generation
    } else if (name === "subcategoryId") {
      newData.subcategoryId = value;
      newData.childcategoryId = "";
      if (value) fetchChildcategoriesBySubCategory(value);
      setSlugEdited(false); // Resume auto-generation
    } else if (name === "childcategoryId") {
      newData.childcategoryId = value;
      setSlugEdited(false); // Resume auto-generation
...
const handleTagsChange = useCallback((e) => {
  ...
  if (value.includes(',')) {
    ...
    if (newTags.length > 0) {
      setFormData((prev) => ({ ...prev, tagsKeywords: [...prev.tagsKeywords, ...newTags] })); // No slice limit
    }
    e.target.value = '';
  }
}, []);
...
const handleTagsKeyDown = useCallback((e) => {
  if (e.key === 'Enter') {
    ...
    if (value) { // Removed length limit
      setFormData((prev) => ({ ...prev, tagsKeywords: [...prev.tagsKeywords, value] })); // No slice limit
      e.target.value = '';
    }
  }
}, []);
...
const handleMetaKeywordsChange = useCallback((e) => {
  ...
  if (value.includes(',')) {
    ...
    if (newKeywords.length > 0) {
      setFormData((prev) => ({ ...prev, metaKeywords: [...prev.metaKeywords, ...newKeywords] })); // No slice limit
    }
    e.target.value = '';
  }
}, []);
...
const handleMetaKeywordsKeyDown = useCallback((e) => {
  if (e.key === 'Enter') {
    ...
    const value = e.target.value.trim();
    if (value) { // Removed length limit
      setFormData((prev) => ({ ...prev, metaKeywords: [...prev.metaKeywords, value] })); // No slice limit
      e.target.value = '';
    }
  }
}, []);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const response = await axios.post(URLS.AddShoppingItem, fd, {
    ...
    [formData, mainImage, additionalImages, video, colorImages, token, navigate, validateForm],
  );
...
<Grid item xs={12} md={6}>
  ...
  <Box sx={{ 
    ...
    {isEditingSlug ? (
      // Edit Mode: Full URL editable
      <CustomTextField
        id="slug"
        name="slug"
        value={`https://doorstephub.com/${formData.slug || ""}`}
        onChange={(e) => {
          let value = e.target.value;
          // Remove domain if user includes it
          value = value.replace(/^https?:\/\/(www\.)?doorstephub\.com\/?/i, '');
          setFormData(prev => ({ ...prev, slug: value }));
        }}
        placeholder="https://doorstephub.com/subcategory-productname"
        fullWidth
        autoFocus
        inputProps={{ maxLength: 90 }}
      />
    ) : (
      // Read-Only Mode: Show full URL in box
      <Box sx={{
        flex: 1,
        display: 'flex',
        alignItems: 'center',
        ...
        >
          https://doorstephub.com/{formData.slug || 'category-subcategory-productname'}
        </Typography>
      </Box>
    )}
  ...
  <FormHelperText>
    {isEditingSlug 
      ? "Edit the full URL path - click ✓ to save" 
      : ""
    }
  </FormHelperText>
</Grid>
              ...
              <Grid item xs={12}>
                <CustomFormLabel htmlFor="tagsKeywords">Tags / Keywords</CustomFormLabel>
                <Box
                  sx={{
                    display: 'flex',
                    flexWrap: 'wrap',
  ...
  <Box sx={{ 
    ...
    <CKEditor
      ...
      config={{
        placeholder: 'Detailed specs, features, and information with rich formatting...',
        toolbar: [
          'heading', '|',
  ...
  <FormHelperText>
    Use the rich text editor to format your product description
                ...
                <Card sx={{ mb: 3 }}>
                  ...
                  <CardContent sx={{ p: 3 }}>
                    <Stack spacing={2}>
                      ...
                      />
                      {errors.mainImage && <Alert severity="error">{errors.mainImage}</Alert>}
                      {mainImagePreview && (
                        <Box>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                ...
                <CustomTextField
                  ...
                  onChange={handleChange}
                  placeholder="0"
                  inputProps={{ min: 0, step: 0.01, 'aria-label': 'Discount Value' }}
                />
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
        ...
        {/* duplicated specifications for the testing */}
        {/* <Box>
          <ParentCard title="Variations" sx={{ mt: 3 }}>
            <Grid container spacing={3}>
                  ...
                  <Grid key={variation._id} item xs={12} sm={6} md={3}>
                    ...
                    <FormControl fullWidth>
                      <CustomSelect
                        id={variation._id}
                        name={variation._id}
                        value={testformData[variation._id] || ''}
                        onChange={testHandleChange}
                        displayEmpty
                        renderValue={(selected) => {
                          if (!selected) return `Select ${variation.name}`;
                          const selectedOption = subVariations.find((sub) => sub._id === selected);
                          return selectedOption ? selectedOption.name : `Select ${variation.name}`;
                        }}
                      >
        ...
        <Box>
  <ParentCard title="Common Specifications" sx={{ mt: 3 }}>
    <Grid container spacing={3}>
      {variations.map((variation) => {
        const relatedSubVariations = subVariations.filter(
          (sub) => sub.shopVariationId === variation._id
        ...
        <Box>
          <ParentCard title="Shipping & Delivery" sx={{ mt: 3 }}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <CustomFormLabel htmlFor="shippingCharges">
              ...
              <Grid item xs={12} sm={6} md={3}>
                <CustomFormLabel htmlFor="returnPolicy">
              ...
              <Grid item xs={12} sm={6} md={3}>
                <CustomFormLabel htmlFor="returnReasonOptions">
                  Return Reason Options <span style={{ color: 'red' }}>*</span>
                </CustomFormLabel>
                <Typography
                  variant="caption"
                  color="text.secondary"
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                <FormControl fullWidth>
                  <CustomSelect
                    ...
                    renderValue={(selected) =>
                      selected
                        .map((id) => colors.find((color) => color._id === id)?.name)
                        .join(', ')
                    }
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                <FormControl fullWidth>
                  <CustomSelect
                    ...
                    renderValue={(selected) =>
                      selected
                        .map((id) => zones.find((zone) => zone.id === id)?.zoneName)
                        .join(', ')
                    }
