...
const BCrumb = [{ to: '/', title: 'Home' }, { title: 'Shopping Request List' }];
...
const CategoriesRequest = () => {
    ...
    if (window.confirm(`Are you sure you want to approve "${categoryData.name}" category?`)) {
      ...
      try {
        const res = await axios.put(
          `${URLS.UpdateShopingItemRequest}/${categoryData.productId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await axios.put(
        `${URLS.UpdateShopingItemRequest}/${selectedCategory.productId}`,
        {
        ...
        renderCell: (params) => {
          return (
            <Box display="flex" gap={1}>
              {rolesAndPermission.shopping_items_request_edit === true ||
              ...
              {rolesAndPermission.shopping_items_request_delete === true ||
              rolesAndPermission.accessAll === true ? (
                <>
                  <Button
                    size="small"
  ...
  return (
    <PageContainer
      title="Shopping Request List"
      description="Review and manage Shopping Request List for your e-commerce platform"
    >
      <Breadcrumb title="Shopping Request List" items={BCrumb} />
      ...
      >
        <Box
          ...
          gap={2}
        >
          <Typography variant="h6">Shopping Request List </Typography>
          <Box display="flex" gap={2} alignItems="center">
            <TextField
              ...
              aria-label="Search Shopping Request List"
            />
          </Box>
        </Box>
        <Divider />
